...
<header>
  ...
  <div class="nav-links">
    {% if user %}
      ...
      <a href="{{url_for('rescan')}}">Rescan Library</a>
...
def rescan():
    ...
    media_count_before = query_db('SELECT COUNT(*) as count FROM media', one=True)['count']
    
    scan_media()
    
    media_count_after = query_db('SELECT COUNT(*) as count FROM media', one=True)['count']
    new_files = media_count_after - media_count_before
    
    if new_files > 0:
        flash(f'Library rescan complete! Found {new_files} new file(s).')
    else:
        flash('Library rescan complete! No new files found.')
...
if __name__ == '__main__':
    print('=' * 60)
    print('MiniFlix Media Server - Starting Up')
    print('=' * 60)
    ...
    if not MEDIA_DIR.exists():
        print(f'WARNING: Media directory does not exist!')
    ...
    with app.app_context():
        ...
        video_files = [f for f in supported_files if f.is_file() and f.suffix.lower() in ALLOWED_EXT]
        ...
        if media_count_after["count"] > 0:
            ...
            print('  Supported formats: .mp4, .mkv, .webm, .avi, .mp3, .ogg')
